...
class PublishingRunner(TaskRunner):
    ...
    def _fix_future_dates_deterministic(self, date_issues: list, repo_root: Path) -> int:
        """
        Deterministically fix future dates by setting them to today.
        ...
        articles_json = self.article_manager.get_articles_json_path(repo_root)
        if not articles_json:
            console.print("    [red]Cannot find articles.json[/red]")
            return 0
        ...
        content_dir = self.article_manager.get_content_dir(repo_root)
        if not content_dir:
            console.print("    [red]Cannot find content directory[/red]")
            return 0
        
        for issue in date_issues:
            article_id = issue.get("id")
            
            # Find article in articles.json
            article = None
            for a in articles:
                if a.get("id") == article_id:
                    article = a
            ...
            # Only fix draft articles
            if article.get("status") != "draft":
                console.print(f"    [dim]Skipping article {article_id} (not a draft)[/dim]")
                continue
            
            old_date = article.get("published_date", "")
            
            # Update content file frontmatter first, then persist JSON to keep both in sync
            file_path = article.get("file", "")
            full_path = self._resolve_article_file_path(file_path, repo_root, content_dir)
            if not full_path:
                console.print(f"    [yellow]⚠ Article {article_id}: Missing file path in articles.json[/yellow]")
                continue
            if not full_path.exists():
                console.print(f"    [yellow]⚠ Article {article_id}: Content file not found at {full_path}[/yellow]")
                continue

            try:
                content = full_path.read_text()
                updated_content, changed = update_frontmatter_date(content, today_str)
                if not changed:
                    console.print(
                        f"    [yellow]⚠ Article {article_id}: No frontmatter date field updated in {full_path.name}[/yellow]"
                    )
                    continue

                full_path.write_text(updated_content)
                article["published_date"] = today_str
                fixed_count += 1
                console.print(f"    [green]✓ Article {article_id}: {old_date} → {today_str}[/green]")
            except Exception as e:
                console.print(f"    [red]✗ Article {article_id}: Error updating file: {e}[/red]")
        ...
        if fixed_count > 0:
            try:
                ...
                console.print(f"    [dim]Saved articles.json[/dim]")
            except Exception as e:
...
Report what structural issues were found and fixed."""
        ...
        # Handle date issues - offer deterministic fix for future dates only
        ...
        if future_date_issues:
            ...
            console.print(f"  2. Let agent handle date fixes")
            console.print(f"  3. Skip date fixes (may fail publishing)")
            ...
            if choice == "1":
                ...
                # Remove fixed date issues from the list
                issues = other_issues
            elif choice == "3":
                console.print(f"[yellow]⚠ Skipping date fixes - publishing may fail[/yellow]")
