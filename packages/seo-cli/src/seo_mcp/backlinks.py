...
def get_backlinks_overview(signature: str, valid_until: str, domain: str) -> Optional[Dict[str, Any]]:
    ...
    """
    if not signature or not valid_until:
        logger.error("No signature or valid_until supplied for domain '%s'", domain)
        return None
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                "Failed to get backlinks overview for domain '%s' (status=%s, response=%s)",
                domain,
        ...
        logger.exception("Exception while fetching backlinks overview for '%s'", domain)
        return None
